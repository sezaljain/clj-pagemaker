...
(defn unpack
  ([input pos fmt number]
   (for [i (range number)]
     (unpack input (+ (* i (con/fmts fmt))  pos) fmt)))
  ([input pos fmt]
   (let [endian      @(:endian input)
         x-bytes     (con/fmts fmt)
         byte-arr    (get-x-bytes-at-pos (:buffer input) pos x-bytes)
